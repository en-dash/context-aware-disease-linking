...
A very straight-forward strategy is direct dictionary lookup using exact \citep[\eg Neji,][]{campos-et-al:2013:neji} or fuzzy matching \citep[\eg][]{ghiasvand-kate:2014:SemEval}.
...
classification:
- dict vs. rules vs. machine learning
- local vs. global vs. graphical model

Entity linking is closely related to named entity recognition (NER) – the detection of textual mentions of a given entity type –, to which it can be seen as a follow-up task.
usually modeled as pipeline with NER
joint:
- plain dictionary approaches: OGER, tagger (Jensen), Neji (without Gimli) -> require blacklists for precision
- joint training, but separate model parameters: TaggerOne
- joint model: JLink
- Lou et al?
